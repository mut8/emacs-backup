...
#+begin_src emacs-lisp
    ...
    (defun my-auto-tex-cmd ()
      "Automatically run latex, pdflatex, or xelatex."
        ;; default command: oldstyle latex via dvi
        (setq texcmd "latexmk -c -f -dvi -pdfps -quiet %f;")        
        ;; pdflatex -> .pdf
        (if (string-match "LATEX_CMD: pdflatex" (buffer-string))
          (setq texcmd "latexmk -c -f -pdf -quiet %f;"))
        ;; xelatex -> .pdf
        (if (string-match "LATEX_CMD: xelatex" (buffer-string))
              (setq texcmd "latex -c -f -pdf -pdflatex=xelatex -quiet %f;"))
        ;; LaTeX compilation command
        (setq org-latex-to-pdf-process (texcmd)))
